...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if (ostraToken != nil && ostraUserID != nil){
            appState.isUserLogged = true
        }
        
        if UserDefaults.standard.string(forKey: "ostraUserID") != nil {
            let appleIDProvider = ASAuthorizationAppleIDProvider()
            appleIDProvider.getCredentialState(forUserID: UserDefaults.standard.string(forKey: "appleID") ?? "") { (state, error) in
    ...
    fileprivate func setupAppareance() {
        ...
        UITabBar.appearance().unselectedItemTintColor = UIColor(named: "textUnselected")
        
        UISegmentedControl.appearance().selectedSegmentTintColor = UIColor(named: "primary")
        UISegmentedControl.appearance().setTitleTextAttributes([.foregroundColor: UIColor.white], for: .selected)
        UISegmentedControl.appearance().setTitleTextAttributes([.foregroundColor: UIColor(named: "primary") ?? UIColor.green], for: .normal)
    }
}
